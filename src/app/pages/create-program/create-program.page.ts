...
export class CreateProgramPage {
            ...
            .then(
              ...
              (err) => {
                ...
                this.alertServ.showToast(
                  'Ошибка загрузки изображения' + JSON.stringify(err)
                )
              }
            )
        } else {
  ...
  createProgram() {
    ...
    if (this.date.from == '' && this.date.from == '') {
      this.alertServ.showToast(
        'Выберите дату начала и окончания действия программы'
      )
      return false
    }
    if (this.connectivityServ.isOnline()) {
      this.httpClient
        .get(
          this.connectivityServ.apiUrl +
            'programs/add?platform_id=' +
            this.route.snapshot.paramMap.get('platform_id') +
            '&title=' +
            this.programInfo.title +
            '&image=' +
            this.programInfo.image +
            '&start_time=' +
            this.date.from +
            '&end_time=' +
            this.date.to +
            '&team_amount=' +
            this.programInfo.team_amount +
            '&max_peoples=' +
            this.programInfo.max_peoples
        )
        .subscribe(
          (data: any) => {
