        ...
        this.httpClient.get(this.connectivityServ.apiUrl + 'news/list?token=' + this.customerData.token + '&type=individual').subscribe((data: any) => {
          ...
          this.httpClient.get(this.connectivityServ.apiUrl + 'routes/list').subscribe((data: any) => {
           ...
           for (let i = 0; i < data.result.routes.length; i++) {
             this.listData.push({ id: data.result.routes[i].id, img: data.result.routes[i].image, title: data.result.routes[i].short_description, subTitle: data.result.routes[i].title });
           }
          }, error => {
            console.log(error);
          });
