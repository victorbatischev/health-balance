...
export class LessonFormPage {
  ...
  task: { program_id: string, title: string, description: string, type: string, video: string, question: string, answers: any, qr_code: string, start_date: string, end_date: string, score: number } = { program_id: '0', title: '', description: '', type: '0', video: '', question: '', answers: [{ answer: '' }], qr_code: '', start_date: '', end_date: '', score: 0 };
  ...
  createLesson() {
    ...
    if (this.connectivityServ.isOnline()) {
       this.httpClient.get(this.connectivityServ.apiUrl + 'lessons/new?program_id=' + this.task.program_id + '&title=' + this.task.title  + '&description=' + this.task.description + '&type=' + this.task.type + '&video=' + this.task.video + '&question=' + this.task.question + '&answers=' + answers + '&correct_answer=' + this.correct_answer + '&qr_code=' + this.task.qr_code + '&start_date=' + this.task.start_date + '&end_date=' + this.task.end_date + '&score=' + this.task.score).subscribe((data: any) => {
         this.alertServ.showToast('Задание было успешно добавлено');
         this.task = { program_id: '0', title: '', description: '', type: '0', video: '', question: '', answers: [{ answer: '' }], qr_code: '', start_date: '', end_date: '', score: 0 };
         this.correct_answer = 0;
         this.navCtrl.pop();
        }, error => {
          console.log(error);
