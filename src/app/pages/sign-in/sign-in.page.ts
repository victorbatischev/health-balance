...
export class SignInPage {
  ...
  doLogin() {
    ...
    if (this.connectivityServ.isOnline()) {
      this.storage.get('deviceToken').then((deviceToken) => {
        this.httpClient.get(this.connectivityServ.apiUrl + 'account/login?email=' + this.loginInfo.email + '&password=' + this.loginInfo.password + '&device_token=' + deviceToken).subscribe((data: any) => {
      ...
      buttons: [
        {
          ...
          handler: (data) => {
            if (data.email.length < 6) {
              ...
              if (this.connectivityServ.isOnline()) {
                console.log(this.connectivityServ.apiUrl + 'account/restore_password?email=' + data.email);
                var email = data.email;
                this.httpClient.get(this.connectivityServ.apiUrl + 'account/restore_password?email=' + email).subscribe((data: any) => {
                  this.showDone();
                  this.loginInfo.email = email;
                }, error => {
                  this.showError('Пользователь с таким e-mail не существует!', data.email);
                });
              } else {
                this.alertServ.showToast('Нет соединения с сетью');
              }
  ...
  async showDone() {
    const alert = await this.alertController.create({
      message: 'На e-mail, указанны при регистрации, было отправлено письмо с инструкцией по восстановлению пароля. В случае отсутствия письма, рекомендуем проверить папку «Спам».',
      cssClass: 'main-alert',
      buttons: [
        {
          text: 'Понятно',
